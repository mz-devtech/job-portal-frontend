...
export default function CandidateSidebar() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="fixed left-0 top-16 hidden lg:flex h-[calc(100vh-4rem)] w-[280px] flex-col border-r border-indigo-100 bg-white/95 backdrop-blur-sm shadow-lg shadow-indigo-500/5 px-5 py-6 transition-all duration-300 z-40">
        ...
        <div className="mt-auto pt-4 border-t border-indigo-100">
          <button className="group relative w-full flex items-center gap-3 rounded-xl px-4 py-3 text-sm text-gray-600 hover:bg-gradient-to-r hover:from-red-50 hover:to-rose-50 hover:text-red-600 transition-all duration-300 overflow-hidden">
      ...
      {/* Tablet Sidebar */}
      <aside className="fixed left-0 top-16 hidden md:flex lg:hidden h-[calc(100vh-4rem)] w-[220px] flex-col border-r border-indigo-100 bg-white/95 backdrop-blur-sm shadow-lg shadow-indigo-500/5 px-4 py-5 transition-all duration-300 z-40">
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.href}
                      href={item.href}
                      className={`group relative flex items-center justify-between rounded-lg px-3 py-2 text-xs transition-all duration-300 ${
                        active 
        ...
        <div className="mt-auto pt-3 border-t border-indigo-100">
          <button className="group relative w-full flex items-center gap-2 rounded-lg px-3 py-2 text-xs text-gray-600 hover:bg-gradient-to-r hover:from-red-50 hover:to-rose-50 hover:text-red-600 transition-all duration-300">
          ...
          {menuItems.flatMap(section => section.items).slice(0, 4).map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`relative flex flex-col items-center justify-center py-1 px-1 rounded-lg transition-all duration-300 flex-1 max-w-[70px] ${
                  active ? 'text-indigo-600 scale-105' : 'text-gray-500 hover:text-indigo-500'
                }`}
              >
