    ...
    <nav className="sticky top-0 z-50 h-14 sm:h-16 bg-white/95 backdrop-blur-md border-b border-indigo-100 shadow-lg shadow-indigo-500/5">
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex h-full items-center gap-3 sm:gap-4 md:gap-6">
          ...
          <Link 
            href="/" 
            className="group relative text-base sm:text-lg md:text-xl font-bold whitespace-nowrap"
          >
              ...
              {navItems.map((item) => {
                ...
                return (
                  <li key={item.label} className="relative group">
                    <Link
                      href={item.href}
                      className={`relative flex h-full items-center px-3 xl:px-4 text-sm font-medium transition-all duration-300 ${
                        active
                          ? "text-indigo-600"
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {!isMobile && (
            <div className="relative group hidden sm:block">
              <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-full border border-indigo-100 hover:from-indigo-100 hover:to-purple-100 transition-all duration-300 cursor-default">
              ...
              <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-[10px] sm:text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none whitespace-nowrap">
                24/7 Support
          ...
          <div className="relative" ref={langRef}>
            <button
              ...
              className="flex items-center gap-1 sm:gap-2 rounded-full border border-indigo-200 px-2 sm:px-3 py-1.5 text-xs sm:text-sm text-gray-700 hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 focus:outline-none focus:ring-2 focus:ring-indigo-300 transition-all duration-300 shadow-sm hover:shadow"
          ...
          {/* HAMBURGER MENU BUTTON - Visible on mobile AND tablet (below 1024px) */}
          <button
            id="mobile-menu-btn"
        ...
        >
          <div className="border-t border-indigo-100 px-4 sm:px-6 py-4 max-h-[calc(100vh-3.5rem)] sm:max-h-[calc(100vh-4rem)] overflow-y-auto">
            
            {/* Phone Number for Mobile only (since tablet already shows it in header) */}
            {isMobile && (
              <div className="mb-4 group flex items-center justify-center gap-2 text-gray-700 py-2 px-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl">
                ...
                return (
                  ...
                  >
                    <Link
                      href={item.href}
                      onClick={() => setMenuOpen(false)}
                      className={`flex w-full items-center justify-between rounded-xl px-4 py-2.5 sm:py-3 text-sm sm:text-base font-medium transition-all duration-300 ${
                        active
            ...
            <div className="mt-6 space-y-3 border-t border-indigo-100 pt-4">
              <p className="px-4 text-xs sm:text-sm font-medium text-gray-500 flex items-center gap-2">
              ...
              <div className="space-y-1">
                {languages.map((lang, index) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-xl px-4 py-2.5 sm:py-3 text-left text-sm sm:text-base transition-all duration-300 transform ${
