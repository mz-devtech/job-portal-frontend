...
const EmployerNavbar = () => {
  ...
  const getVisibleNavItems = () => {
    if (!isMobile && !isTablet) {
      ...
      return navItems.slice(0, 3); // Tablet - show first 3
    }
  ...
  return (
    <nav className="sticky top-0 z-50 h-14 sm:h-16 bg-white/95 backdrop-blur-md border-b border-indigo-100 shadow-lg shadow-indigo-500/5">
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-3 xs:px-4 sm:px-6 lg:px-8">
        
        {/* LEFT SECTION */}
        <div className="flex h-full items-center gap-2 xs:gap-3 sm:gap-4 md:gap-6">
          {/* Logo */}
          <button 
          ...
          {/* Desktop/Tablet Navigation - Hidden on mobile, visible on tablet and desktop */}
          {!isMobile && (
                ...
                return (
                  <li key={item.label} className="relative group">
                    <button
                      ...
                      className={`relative flex h-full items-center px-2 lg:px-3 xl:px-4 text-xs lg:text-sm font-medium cursor-pointer transition-all duration-300 w-full text-left ${
                        active
                          ? "text-indigo-600"
                    ...
                    >
                      ...
                      {!profileComplete && item.requiresCompleteProfile && (
                        <>
                          <span className="ml-1 text-[10px] lg:text-xs text-amber-500 animate-pulse">●</span>
                          <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-amber-400 to-orange-400 text-white text-[10px] lg:text-xs px-2 lg:px-3 py-1 lg:py-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-50 shadow-lg scale-90 group-hover:scale-100 hidden lg:block">
                            <div className="flex items-center gap-1">
                              <span className="animate-pulse">⚠️</span>
                              Complete profile ({profilePercentage}%)
                            </div>
        ...
        <div className="flex items-center gap-1 xs:gap-2 sm:gap-3">
          ...
          <div className="relative" ref={langRef}>
            <button
              ...
              className="flex items-center gap-1 sm:gap-2 rounded-full border border-indigo-200 px-2 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm text-gray-700 hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 focus:outline-none focus:ring-2 focus:ring-indigo-300 transition-all duration-300 shadow-sm hover:shadow"
        ...
        {/* HAMBURGER MENU OVERLAY - For mobile AND tablet */}
        {menuOpen && (
          <div 
            className="fixed inset-0 top-14 sm:top-16 z-40 bg-gradient-to-b from-black/20 to-black/10 backdrop-blur-sm animate-fadeIn lg:hidden" 
            onClick={() => setMenuOpen(false)} 
          />
        )}

        {/* HAMBURGER MENU CONTENT - For mobile AND tablet */}
        <div
          ref={menuRef}
          className={`fixed left-0 right-0 top-14 sm:top-16 z-40 bg-white/95 backdrop-blur-md shadow-2xl shadow-indigo-500/10 transition-all duration-500 ease-out lg:hidden ${
            menuOpen
              ? "translate-y-0 opacity-100"
              : "-translate-y-8 opacity-0 pointer-events-none"
          }`}
        >
          <div className="border-t border-indigo-100 px-3 xs:px-4 sm:px-6 py-3 xs:py-4 max-h-[calc(100vh-3.5rem)] sm:max-h-[calc(100vh-4rem)] overflow-y-auto">
            
            {/* Phone Number for Mobile only */}
            {isMobile && (
              <div className="mb-4 group flex items-center justify-center gap-2 text-gray-700 py-2 px-3 sm:px-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl">
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className={`flex w-full items-center justify-between rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base font-medium transition-all duration-300 ${
                        active
                          ...
                          : "text-gray-600 hover:bg-gradient-to-r hover:from-indigo-50/50 hover:to-purple-50/50 hover:pl-5 sm:hover:pl-6"
                      }`}
                    ...
                    >
                      ...
                      {!profileComplete && item.requiresCompleteProfile && (
                        <span className="text-[10px] sm:text-xs px-2 py-1 bg-amber-100 text-amber-700 rounded-full animate-pulse">
                          {isMobile ? "🔒" : "Locked"}
                        </span>
                      )}
                    </button>
            ...
            <div className="mt-5 sm:mt-6 space-y-2 sm:space-y-3 border-t border-indigo-100 pt-4 sm:pt-5">
                ...
                {languages.map((lang, index) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-2 sm:gap-3 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-left text-sm sm:text-base transition-all duration-300 transform ${
                      ...
                      activeLang.label === lang.label
                        ? "bg-gradient-to-r from-indigo-50 to-purple-50 text-indigo-700"
                        : "text-gray-700 hover:bg-gradient-to-r hover:from-indigo-50/50 hover:to-purple-50/50 hover:pl-5 sm:hover:pl-6"
                    }`}
                  >
                    <img 
                      src={lang.flag} 
